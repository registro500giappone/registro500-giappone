/* ストーリー第7号「ホーンが鳴らない」の絵。journey_id: horn_silent（URLと対＝変えない）
   共通の土台（場面の作り方・描画プリミティブ・共通部品・実車図・検算・起動）は /wiring-journey.js。
   ⚠️図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。

   このストーリーだけの特徴＝【2つとも、これまでのストーリーとは逆】。
     ①ヒューズが本線に入る。第1・4・6号はどれも「ヒューズは無実」だった（キー・コイルへの線は
       ヒューズの電源側から分かれる）。ホーンは F1 の【負荷側】に着く初めての系統＝
       ヒューズを疑うのが正しい数少ない症状。
     ②スイッチがアース側に入る。ホーンには常に12Vが来ていて、ボタンは【帰り道】をつなぐ。
       だからキーを抜いても鳴る。
   ⛔ホーン本体の「通電しても鳴らない・かすれる」（調整ねじ・内部接点の荒れ）は値の話＝この絵の外。

   ⚠️系統5 を新設したストーリー＝NET_VERSION 5→6（部品 horn・horn_sw、電線 w05-01/02/03）。 */
(function () {
  'use strict';
  var WC = Journey.WC, C = Journey.C;
  /* ⚠️配置の決めごと（第4・6号で確立した作法をそのまま使う）：
       ①部品の箱の中は【左＝端子名／中央 x=76 の縦軸＝内部の線／右＝文字】の3列に分ける。
       ②ヒューズは縦向き（k.fuseV）＝第4号の横向きと違う。本文でそのことを断ってある。 */
  var X = 76;
  ...
  function draw(k, mode) {
    ...
    /* 赤地に白抜きの札＝この絵でいちばん見てほしい所に1枚だけ置く（第4・6号と同じ道具）。
    ...
    /* 【赤の序列】切断は すき間13・×・ハロー・図幅いっぱいの境界破線・赤札1枚（2026-08-24・第4号）。
       ⚠️赤はこの「切れている場所」ただ1つに取っておく。
       ⚠️切断を描く区間は 56px 以上空ける＝ハロー（r=19）が隣の記号と食い合う（第6号の実測）。 */
    ...
    /* ===== バッテリー〜ヒューズ箱の電源側までは第1〜4号でたどった道 ===== */
    k.battery(X);
    /* ⭐端子バッジ＝原典に番号のある端子にだけ付ける（2026-08-24・第4号の作法／第6号で全号展開）。
       この絵で付けるのは + / 30（レギュレータ）/ 30（ヒューズ箱の電源側）/ ホーンの + ・− の5か所。
       ⚠️F1の負荷側とホーンボタンの接点には付けない＝原典に名前が無いから（絵の中で言い切ってある）。
       ⚠️ホーンの +・− は原典の配線図に番号としては出てこないが、実車の端子には刻印があり、
         本文がそこへテスターを当てろと言っている＝図と本文を1対1にするために出す（第6号と同じ扱い）。 */
    k.term(X, 74, '+', 'l');
    ...
    /* ===== ヒューズ F1＝ここが本線。⚠️第4号ではこれを横の枝として描いた ===== */
    k.node(X, FZ);
    /* キー系への分岐＝ヒューズの【手前】から出る＝F1が切れてもキー系は無事。
       ⚠️分岐の文字は箱（x+62=138まで）と重ならない位置＝x=150 以降に置く。 */
    k.dashOut(X, FZ, 146);
    k.label(150, FZ - 6, 'キースイッチへ', C.sub, null, 10);
    k.label(150, FZ + 7, '（第1・4・6号）', C.sub, null, 10);
    k.term(X, FZ - 10, '30', 'l');
    k.fuseV(X, FZ, blown, 'ヒューズ F1');
    /* ⛔ヒューズが切れている場面に赤札は出さない（実測して取り下げた）＝k.fuseV が
         エレメントの分断・赤い丸・「切れている」の状態語まで既に描いていて、札を足すと
         同じことを2回言うことになる（画面で見て初めて分かった）。
       💡【線の上に切断が無い場面こそ印が要る】（第4号）が効くのは、**どの線も生きて見える**
         場面＝ここは部品そのものが切れた姿で描かれているので、その条件に当たらない。 */
    ...
    /* 内部＝電磁石の巻線（第6号のコイルと同じ描き方に揃える＝読者が同じ物として読める）。
       ⚠️これは電線ではないので被覆色を付けない＝明るい内部線 */
    ...
    /* ⚠️＋と−は本文の絞り込みで【実際にテスターを当てる】2つ＝hero（濃く反転）で出す。 */
    k.term(X, HB.y - 12, '+', 'l', true);
    k.term(X, HB.y + HB.h + 10, '−', 'l', true);
    /* 振動板＝箱の右側。鳴っているときだけ音の弧を出す（⛔点滅させない）。
       ⚠️状態の語（鳴っている／鳴らない）は箱の【外・右】に置く＝キースイッチと同じ作法。
         箱の中に入れると音の弧に串刺しにされる（実測して直した）。 */
    ...
    if (mode.cut === 'w05-02') cutV(HB.y + HB.h, BB.y, 'w05-02');
      ...
      /* コラムのコネクタ＝原典にインラインコネクタ記号がある所（NL-D の記載）。
         ⚠️コミュテータ（部品11）の接点は通らない＝束を共にするだけ、と本文で断る。 */
    ...
    k.label(6, 664, '⬆ 車体からバッテリーの − へ帰って、輪が閉じる（第5号）。', C.sub, null, 10.5);
  ...
  /* ---- トグル（ボタン 離す ↔ 押す）＝キーは OFF のまま動かさない ----
     ⭐このストーリーの主張＝「キーを抜いていても鳴る」。トグルの inputs で key を OFF に固定してある。 */
  var CAPS = {
    ...
    OFF: '<b>ボタンを離した＝帰り道が切れました。</b>ホーンの ＋ 端子にはこの瞬間も12Vが来ています（線の色が付いたままなのはそのためです）。<b>電圧は来ているのに鳴らない</b>——これが正常な状態です。テスターを当てると、ホーンの両方の端子で12Vが読めます。'
  };
  ...
  var CHECKS = [
    ...
    /* アースの帰り道（第5号）が切れるとホーンも死ぬ */
    { label: 'バッテリー−（w11-10）が外れた・押す', s: { inputs: PRESS, ops: cut('w11-10') }, expect: false, words: HW },
