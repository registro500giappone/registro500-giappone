/* ストーリー第7回「ホーンが鳴らない」の絵。journey_id: horn_silent（URLと対＝変えない）
   ...
   このストーリーだけの特徴＝【2つとも、これまでのストーリーとは逆】。
     ①ヒューズが本線に入る。第1・4・6回はどれも「ヒューズは無実」だった（キー・コイルへの線は
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* 【赤の序列】切断は すき間13・×・ハロー・図幅いっぱいの境界破線・赤札1枚（2026-08-24・第4回）。
       ⚠️赤はこの「切れている場所」ただ1つに取っておく。
       ⚠️切断を描く区間は 56px 以上空ける＝ハロー（r=19）が隣の記号と食い合う（第6回の実測）。 */
    ...
    k.battery(X);
    /* ⭐端子バッジ＝原典に番号のある端子にだけ付ける（2026-08-24・第4回の作法／第6回で全号展開）。
       ...
       ⚠️ホーンの +・− は原典の配線図に番号としては出てこないが、実車の端子には刻印があり、
         本文がそこへテスターを当てろと言っている＝図と本文を1対1にするために出す（第6回と同じ扱い）。 */
    k.term(X, 74, '+', 'l');
    ...
    /* ===== ヒューズ F1＝ここが本線。⚠️第4回ではこれを横の枝として描いた ===== */
    k.node(X, FZ);
    ...
    /* ⛔ヒューズが切れている場面に赤札は出さない（実測して取り下げた）＝k.fuseV が
       ...
       💡【線の上に切断が無い場面こそ印が要る】（第4回）が効くのは、**どの線も生きて見える**
