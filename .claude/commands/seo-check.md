...
## ステップ0: まず機械に数えさせる（必須・目視の前に）

```bash
python py/check_pages.py
```

全HTMLの `<head>` を走査し、**GA4タグの欠落・測定IDの誤り**（NG＝終了コード1）と、canonical・og:image の欠落（注意）を出す。**新しく作ったHTMLは何もしなくても検査対象に入る**（検査しないものだけを `py/check_pages_ignore.json` に列挙する方式）。

- ⛔**公開しているページを、警告がうるさいという理由で除外リストへ足さない。**
- ⚠️**GA4タグは目視のチェックリストから何度も漏れている**（2026-09-06＝マイ壁紙とスクリーンセーバーの2本が、公開後まったく計測されていなかった）。canonical と OGP は公開作業の一部として意識に上がるが、**GA4タグだけは公開手順のどこにも現れない**。だから人の目ではなくこのコマンドで数える。
- 同じ検品は push のたびに GitHub Actions（`.github/workflows/check-pages.yml`）でも走る。
...
| GA4タグ | `googletagmanager.com/gtag/js?id=G-27SHHC4JYH` の読み込みが `canonical` の直後にある。⚠️**`noindex` のページにも要る**＝noindex は「検索に出さない」であって「人が来ない」ではない（`edit.html` などログイン後の主要画面もそちら側） |
