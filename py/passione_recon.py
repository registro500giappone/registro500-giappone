"""
Passione 500 クローラー（再構築版）
- Selenium廃止 → requests + BeautifulSoup（5〜7倍高速化）
- 推定実行時間: 100〜150分（旧版: 511分）
- GitHub Actions 6h制限内に収まるように
"""
...
def detect_target_cars(name, url):
    """商品名・URLパスから対象車種を検出
    注意: 商品名末尾の "| Passione 500" / "- Passione 500" を除去してから判定
    """
...
def get_all_urls():
    ...
    try:
        ...
        # product_cat（カテゴリ）は除外・商品URLを含まないため
...
def parse_price(price_el):
    """価格要素から数値を抽出（\xa0・€・カンマ対応）"""
    if not price_el:
        return 0.0
    # WooCommerceは sale価格があると複数の.amountが出る → 最後（実売価格）を使用
...
def scrape_product(session, url):
    ...
    try:
        ...
        # 商品名（"- Passione 500" を除去）
        og_title = soup.find('meta', property='og:title')
        if og_title and og_title.get('content'):
            name_en = re.sub(r'\s*[-|]\s*Passione\s*500\s*$', '', og_title['content'], flags=re.IGNORECASE).strip()
        else:
            h1 = soup.find('h1')
        ...
        # 在庫
        stock_status = "在庫あり" if "in-stock" in r.text.lower() else "在庫なし"

        # OEM番号
        oem_no = "N/A"
        meta_el = soup.find(class_='product_meta')
        if meta_el:
            m = re.search(r'(?:Original|OEM|Rif|Ref).*?[:\.]\s*([0-9\s/A-Z\-]{5,})',
                          meta_el.get_text(), re.IGNORECASE)
            if m:
        ...
        return {
            ...
            "image_url": image_url,
            "page_url": url,
            "target_cars": detect_target_cars(name_en, url),
