...
- **【2026-08-24 記録】外国語動画の日本語化ガイド `video-translate.html` を新設（未コミット・導線なし・noindex）**
  - **設計判断（変えない）**：サイトは翻訳を**提供しない**＝ユーザーが自分の環境で翻訳するための**道案内に徹する**。逐語字幕の生成・配信は権利上サイト側では行わない（正確＝逐語寄りほど翻案権に直接当たる。ラベルでなく**処理がユーザー側で完結する実態**で私的利用を成立させる）。
  - **中身**：方法A=YouTube AI吹き替え音声（⚙️→音声トラック。iPhone実機確認済）／方法B=字幕を無料AIに貼る（本命）／方法C=拡張＋自分の無料Geminiキー／字幕なし動画は有料と明記／無料・有料の早見表。
  - **実測の根拠**：YouTube自動翻訳字幕は85断片中69%が文の途中で切れ、sedi→「オフィス清掃」等に崩れる（イタリア語動画 FrAGcnjLiyo）。AI吹き替えは人気12本中6本に日本語音声＝**すべて英語原語**。
  - **鮮度維持**：外部サービスの現状を書くページなので**3ヶ月ごとに再測**（次回目安 2026-11。AI吹き替えの対応範囲が最速で動く箇所なのでこの周期。2回転変化なしなら半年に緩めてよい）。手順と判定基準は `video-translate-RECHECK.md`、再測スクリプトは `recheck_audio_tracks.py`／`recheck_vtt_compare.py`（いずれも本ディレクトリ）。
