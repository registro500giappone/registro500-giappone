/* ストーリー第6回「セルは回るが、かからない（火花が飛ばない）」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   ...
   ここを混ぜると「つながっている＝火花が出る」と読めて、診断の道具として人を誤らせる（JOURNEY-INDEX §4／wiring-net.json の _sys9_scope）。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* 端子バッジ＝原典に番号（記号）のある端子にだけ付ける。この絵で付けるのは + / 30 / 15/54 / コイルの + ・D ・B の6か所。
       ...
       コイルの D・B は原典の配線図では割付を断定できない（端子台帳の）が、実車のコイルには刻印があり、本文がそこへテスターを当てろと言っている＝図と本文を1対1にするために出す。 */
    k.term(X, 74, '+', 'l');
