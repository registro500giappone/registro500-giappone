/* ストーリー第6回「セルは回るが、かからない（火花が飛ばない）」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   このストーリーだけの特徴＝【絵に「外」がある】。点火は2つの輪でできている：バッテリーで回る一次の輪と、その電流が切れた瞬間に誘導で生まれる二次（高圧）の輪。
   L1（到達性）が解けるのは【一次だけ】。だから高圧側は計算せず、右の囲みに「この絵の外」として置く。
   ここを混ぜると「つながっている＝火花が出る」と読めて、診断の道具として人を誤らせる（wiring-net.json の _sys9_scope で、系統9は一次側だけに絞ってある）。
   点火時期・ドエル角は「道は正常なまま値だけが動く」領域＝この絵の外側。本文で境界を書く。 */
(function () {
  'use strict';
  var WC = Journey.WC, C = Journey.C;
  /* ②高圧の囲みは【右横ではなく一番下の帯】。右に置くとキースイッチや電線のラベルと横で押し合って、どちらも読めなくなった。
     下に敷けば横幅を全部使える。 */
  var X = 76;                                  /* 一次の輪の縦軸 */
  /* と切断の札（chip）を入れるため、箱を下へずらして縦の余白を広げた。 */
  /* が端子バッジ「15/54」（y=178）と「+」（CB.y-12）に触れる。 */
  var CB = { x: 16, y: 248, w: 174, h: 90 };   /* コイルの箱 */
  var DB = { x: 16, y: 384, w: 174, h: 68 };   /* デスビ（ポイント）の箱 */

  /* ===== 従図＝「火花ができるまで」（主図のトグルに連動して描き替わる・ ===== 主図の一番下に敷いていた高圧の帯をここへ移し、コイルの中で何が起きているかまで解説する。
  ...
  /* 描く物語は「電気が来る→一次に磁気が溜まる→ポイントが開いて電流が切れる→二次に高圧が生まれる→高圧が線を伝ってプラグまで届く→火花が飛ぶ」。
     だから状態で描き分けない（closed / fire の分岐を捨てた）。動きは全部 CSS の6秒周期に載る。
     初期値（＝アニメが効かない `prefers-reduced-motion: reduce` の読者）は「①電気が流れて磁気が溜まっている」の静止画になるよう opacity 属性を置いてある。
     この属性を消すと、動きを止めている読者に見出しが3枚重なって出る。見出し3枚と右上のラベル2枚は【同じ場所に重ねて置く】＝getBBox の衝突検査に必ず出る。
     意図した重なりなので、検査結果を見るときはこの5枚を除いて数えること。電気の向きは連載の作法どおり【＋から出てアースへ帰る】（第5回と同じ）。 */
  ...
  function drawSpark(k) {
    ...
    /* 磁気＝鉄芯を取り巻く輪。育つ→満ちる→崩れるを1本の keyframes で回す。
       rx は 40 まで＝これ以上広げると左右の巻線に触る（P1=104 / P2=196）。
       2枚の輪に同じクラスを付ける＝ばらばらに動くと「崩れる」に見えない。 */
    ...
    /* 一次巻線＝太い線・巻数は少ない（4巻き）。被覆色を付けない＝電線ではない */
    ...
    /* ---- 高圧の道＝B端子から【コイルの外へ出て】右端を降り、プラグ①②へ配られる。
       ここを1本の線で繋いだのが要点＝「二次に高圧が生まれた」と「プラグが光った」の間を、線が伸びていく動きで結ぶ。
       2つを同時に光らせない。道は【B端子が起点】＝dashoffset を実長ぶん振るので、途中で形を変えたら長さも直すこと。 */
    ...
    /* ---- ポイント＝一次の輪を切る所。コイルの箱の【外】に描く（デスビの中の部品だから）。
       ②開いたときの跳ね上がりを 13px→20px にして、離れたことが遠目にも分かるようにした（そのぶん PY を 204→218 まで下げてある＝上げると開いた腕の玉がコイルの箱に食い込む）③開いている間だけ【橙のリング・大きめの⚡・「ここで切れる」の文字】の3点セットを出す④開いた【瞬間】に橙の波紋を1回はじけさせる（ig-snap）＝「パチンと切れた」を目に留めるここで赤は使わない。
       赤は連載を通して【故障で切れている場所】に取ってある色で、ポイントが開くのは正常な動作だから。
       橙＝高圧まわりの色で統一する。 */
    ...
    /* 流れる点＝丸い破線を動かす（サイクルの前半だけ流れて、ポイントが開いた瞬間に消える）。
       【ポイントより後に描く】＝先に描くとポイントの線（太さ3・濃色）に上書きされて、箱の外を流れる点が1つも見えなくなる。
       【道は車体アースまで通す】＝ポイントの接点(48)で止めると、閉じている間も接点からアースまで電気が流れていないように見える。
       一次はキー→コイル→ポイント→車体アースで輪になって初めて電流が流れる。手前で切らない。
       閉じている腕(60→92)の上を通るが、ig-prim は開いている間は消えるので矛盾しない。 */
  ...
  /* ---- シリンダーヘッドの断面＝高圧がどこへ届き、どこで火花になるか模式図。500 は【空冷の直列2気筒】＝ヘッドは1つ、プラグは2本。
     冷却フィンを描くのは「空冷のヘッド」だと一目で分かるようにするため。ヘッド・燃焼室の寸法は原典に無い＝形を写した図ではないと分かる粗さにとどめる。
     デスビのローターが①と②へ【交互に】配る＝2本が同時に飛ぶことはない。だから絵も【前半サイクルで①・後半サイクルで②】と代わりばんこに光らせている。
     ただし【本文で「2本が同時に飛ぶことはありません」と断らない】＝。同時点火（ウェイストスパーク）に換えている車も多く、わざわざ言うと余計な断りになる。
     絵が交互に飛ばしていること自体は変えない。高圧の道は【B端子から伸びていく】＝二次の稲妻が出てから火花までを線の流れで繋ぐ。
  ...
  function head(k, top) {
    ...
    /* シリンダーの中＝暗く塗る。ここを明るいままにすると火花が背景に埋もれて「派手さ」が出ない。
       ヘッドより先に描いて、ヘッドで上を隠す。 */
    ...
    [[A, '①'], [B, '②']].forEach(function (q) {
      ...
      /* 火花＝白い芯＋橙の外炎＋放射の閃光。高圧の道が届いてから光る（同時ではない） */
  ...
  function draw(k, mode) {
    ...
    /* 線が1本外れている場面の描き方＝【赤の序列】。すき間を広げ・×を打ち・ハローを敷き・図幅いっぱいの破線で「ここから下が死ぬ」境界を引き、赤地に白抜きの札を1枚だけ添える。
       赤はこの「切れている場所」ただ1つに取っておく。opt.mid＝すき間の中心を区間の真ん中から動かす（端子バッジと近づきすぎるのを避ける）。
       札は図の【右の余白】に出す＝左は端子バッジの列（x=26〜67）で埋まっている。 */
    ...
    k.battery(X);
    /* 端子バッジ＝原典に番号（記号）のある端子にだけ付ける。この絵で付けるのは + / 30 / 15/54 / コイルの + ・D ・B の6か所。
       デスビの接点とアースには付けない＝原典に番号が無いから（付けると実車に無い端子を探させる）。
       コイルの D・B は原典の配線図では割付を断定できないが、実車のコイルには刻印があり、本文がそこへテスターを当てろと言っている＝図と本文を1対1にするために出す。 */
    k.term(X, 74, '+', 'l');
    ...
    /* このストーリーが通らない枝＝計器盤の警告灯とヒューズF2。コイルはF2を通らない。
       分岐は y=180（キースイッチのラベル y=152 から十分離す）。166 に置いたら文字が重なった。 */
    k.node(X, 178);
    k.dashOut(X, 178, 150);
    ...
    /* ===== コイル（一次巻線＝この絵で計算している負荷） ===== 3列に分ける：左＝端子名／中央 x=76＝巻線／右＝文字 */
    ...
    /* 一次巻線の記号。これは電線ではないので被覆色を付けない＝明るい内部線で描く（計器盤の中を細い明るい線で描いたのと同じ作法＝原典の電線と見分けが付くように） */
    ...
    /* ＋とDは本文の絞り込みで【実際にテスターを当てる】2つ＝hero（濃く反転）で出す。
       バッジは箱の外（上端-12／下端+12）＝箱の縁や巻線に重ねない。 */
  ...
  /* ---- トグル（ポイント閉↔開）＝このストーリーで人が動かせる唯一のもの ---- 「開いているときに電流が止まっているのが正常」＝ここが第1〜5回と逆で、いちばん誤解される所。 */
  var CAPS = {
  ...
  Journey.boot({
    ...
    /* primaryOk＝「一次の輪が最後までつながっているか」。いま流れているか（coilOn）とは別物で、ポイントが開いている正常な瞬間も true になる。
       これは solve() の端子状態から導いたもので、絵の都合で作った旗ではない：ポイントの手前が hot か post（＝＋側から見えている）で、かつポイントの向こう側が gnd（＝アースまで帰れる）なら、閉じれば必ず流れる。 */
    extra: function (sc) {
    ...
    scenes: function (scenario) {
      return [
        ...
        /* 従図＝火花ができるまで。トグルには連動しない（1周6秒の通しアニメを回し続ける・＝だから scenes に置く。
           sc は使わないが器として渡す。 */
        { id: 'j-spark', sc: scenario({ inputs: OPEN }), mode: { spark: true } }
      ];
    },
    /* 点火の部品はエンジンルームに固まっている＝バッテリーとキーだけが車の前。ので印を打った。
       今回は左後方」。印は実測どおり左後方に打つが、本文の .note で必ず個体差を断ること（読者が自分の車で探すため）。
       ダイナモの印は【外した】＝実測が無かった頃に「位置の目安」として置いていたもので、役目が終わったため（本文にダイナモは1度も出てこない）。
       札はどちらも既定（点の左へ伸ばす）＝右へ出すと図の右端(3020)からはみ出す。 */
    carmap: function () {
      return {
