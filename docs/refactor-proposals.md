...
## P3. セキュリティ境界の既知課題（旧D11・認識済み・コード変更禁止）

既知のセキュリティ境界課題が3件あり、オーナーと開発担当の間で**非公開メモとして管理**している（本リポジトリは公開のため詳細は記載しない）。いずれも修正は挙動変更・スキーマ変更を伴うプロダクト判断案件であり、リファクタリングの「ついで」に触らないこと。

---

## P4. クローラー共通化（D2）— **完了（2026-06-11）**

`crawler_common.py` 新設＋アクティブなクローラー9本すべての移行が完了。経緯:
1. dangelo_recon.py を移行し、push 承認後に crawl-dangelo.yml を workflow_dispatch で実地検証 →
   **run 27347701935 成功・parts テーブルの D'Angelo 件数 2347→2349 件（service_role 書込成功を実証）**。
2. ゲート充足を確認後、残り8本を cherry-pick で適用（5585dad〜d04bae2・各1コミット）。
   各移行は py_compile＋importスモークテスト＋diff目視（リトライ秒数・sleep・UA・upsertキー無変更）で検証済み。
3. passione の `detect_target_cars` は正規表現が異なる（`500` に語境界なし）ため共通化せず独自実装を保持。
4. daily-parts-update.yml（autobella/ricambio/mrfiat）は毎日03:00 JSTの定期実行が継続的な実地検証になる。
   翌日の実行結果（件数が平常水準か）を一度確認することを推奨。
...
### 【回答済み 2026-06-11】D3/D4（spots_api.gs / spots_api.js の削除）→ B: 見送り決定
**オーナー回答: B（spot_schedule.html の方針決定とセットで見送る）。** 両ファイルはリポジトリに残置。
spot_schedule.html の扱い（削除/Supabase化/放置）を決める際に併せて整理すること。以下は判断時の経緯記録。
