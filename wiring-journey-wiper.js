/* ストーリー第11回「ワイパーが動かない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   ...
   スイッチを通らずにモーターの C 端子へ直接1本行っている（原典の色を4型式ともたどって確定＝HANDOFF その129⑪）。
   ②【症状が2つある】＝動かない／止まらない。後者はスイッチの固着で、絵の上でも作れる（第7回のブレーキランプと同じ形）。
   ③【F2 の3人目】＝ブレーキランプ（第7回）・ウインカー（第10回）と同じヒューズ＝この回で3つ揃う。
   ⚠️⚠️【切ったあと定位置まで戻る仕組み（オートストップ）は、この絵では作れない】＝モーターの中のカム接点が要るが、
   原典はモーターの内部を描いていない（4端子の箱のみ）。だから C と F の2本は【つながっていることしか表さない】＝
   黄点を流さない（第10回のフラッシャーのアースと同じ扱い）。⛔原典に無い内部を絵にしない（HANDOFF その125④）。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    label(4, 768, 'BIANCO（白）＝結線はD・F・L・R型とも同じです。', C.sub, null, 10);
