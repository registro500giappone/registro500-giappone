...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* ===== 灯 ===== 前は車幅灯と兼用の2フィラメント球（本社の電球一覧 20W/5W・2026-09-02 確定）・後ろは3球1ケース
       ＝球とケースの話は本文が持つ。⛔部品番号は書かない（資料ごとに番号が割れる）。
    ...
    /* ⛔部品番号は書かない（2026-09-02）＝同じフラッシャーが【本社D図/本社F図=9・本社L図/部品商F図=8】と資料で割れる。
       どの体系でも通用する番号は存在しないので、手がかりは端子名（+・31・P・L）に任せる＝第11回と同じ扱い。 */
    ...
    s.push('<text x="' + XC + '" y="' + (LV_T + 33) + '" font-size="10" fill="' + C.in_ + '" text-anchor="middle">（3つの位置）</text>');
    ...
    if (m.cut === 'w04-06') return warn('⚠️ウインカーレバーから【左前】への1本が外れている。前も横も消え、', '　 後ろだけが残る＝横へは前の灯のところで分かれているから。');
    if (m.cut === 'w04-12') return warn('⚠️左前の灯の【アース】だけが外れている。横も後ろも点いたまま。');
    if (m.f2) return warn('⚠️F2 が切れると、ウインカーもブレーキランプも同時に死ぬ。');
    if (m.dead) return warn('⚠️フラッシャーが死ぬと、ウインカーレバーを倒しても6つとも点かない。', '　 表示灯（緑）も点かない。');
    if (m.off) return warn('⚠️ウインカーレバーが中立＝これは故障ではない。');
    return 980;
  }
