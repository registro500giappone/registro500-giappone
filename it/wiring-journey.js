...
(function (global) {
  ...
  Kit.prototype.suspect = function (x, y, w, h, textY, t, quiet) {
    ...
    this.s.push('<text x="' + (x + 6) + '" y="' + textY + '" font-size="13" fill="' + C.hi + '" font-weight="700"' + (quiet ? ' opacity="0.7"' : '') + '>' + (t || 'Zona sospetta') + '</text>');
  };
  ...
  Kit.prototype.battery = function (x) {
    ...
    s.push('<text x="' + x + '" y="56" font-size="12" fill="#fffdf8" text-anchor="middle">Batteria</text>');
  };
  ...
  Kit.prototype.keySwitch = function (x, top, on, labelX, opts) {
    ...
    s.push('<text x="' + (x + 28) + '" y="' + (top + 28) + '" font-size="10.5" fill="' + C.in_ + '" text-anchor="middle">' + (on ? 'Chiuso' : 'Aperto') + '</text>');
    var lx = (labelX == null) ? x + 56 : labelX;
    this.label(lx, top + 18, 'Commutatore di accensione', C.deep, null, 12);
    this.label(lx, top + 34, on ? 'Chiave su ON' : 'Chiave su OFF', on ? C.deep : C.sub, null, 12);
  };
  ...
  Kit.prototype.fuseV = function (x, top, blown, name) {
    ...
    if (name) {
      ...
      s.push('<text x="' + (x + 24) + '" y="' + (top + 42) + '" font-size="11.5" font-weight="700" fill="'
        + (blown ? C.hi : C.ok) + '">' + (blown ? 'bruciato' : 'integro') + '</text>');
    }
  };
