 ...
 *  ⚠️第1回（チャージランプ）はユーザー確定済み＝このファイルへの切り出しで見た目を1文字も変えないこと。
...
(function (global) {
  ...
  /* quiet＝囲いを一段引く（2026-08-24・第4回）。同じ絵に「切れている場所」が描いてあるとき、
  ...
  /* 端子バッジ＝原典に番号のある端子を、線が着く場所に【同じ形で】出す（2026-08-24・第4回）。
  ...
  /* labelX＝部品名を書き出す x（省略時は従来どおり x+56）。
     ⚠️容疑の囲いがある絵では、部品名が破線に貫かれることがある＝そのページだけ外へ逃がすために足した
        （2026-08-23・第4回。既定値は変えていないので他のストーリーの絵は1文字も変わらない＝回帰で実測済み） */
  /* opts.hero＝そのストーリーの主役の部品として浮き上がらせる（2026-08-24・第4回）。
     ⚠️既定は false＝主役でないストーリー（第3回・第6回…）の絵は1px も変わらない。
  ...
  /* ヒューズを【縦に通す】絵（500のヒューズは筒型＝上下に金属キャップ）。
     ⚠️第4回（キー）はヒューズを「幹線の横にぶら下がる枝」として描いた＝キーへの線がヒューズの
  ...
  /* opts: {viewBox, marks:[{id,color,label,anchor}], legend, scale} — 座標は wiring-layout.json の実測値
     ...
     ⚠️scale は「viewBox をどれだけ引いたか」の補正（既定 1＝engine ルームに寄った第1・2回のまま）。
