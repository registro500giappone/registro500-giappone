...
(function () {
  ...
  // ---- 一時：固定ボタンのズレを実機で調べる（原因が分かったら丸ごと消す） ----
  // 2026-08-12。iOSのPWAで、下部の浮遊ボタンが画面中央付近に居座る症状が出た。
  // 単体の診断ページ(fabdebug.html)では再現せず、実ページでしか数値が取れない。
  // PWAにはURL入力欄が無いので、ボタンの長押し(1.2秒)で診断パネルを出す。
  // パネルは position:absolute（fixed だと調べたい不具合の影響を受けるため）。
  try {
    var diagPanel = null, diagTimer = null;
    var r0 = function (n) { return Math.round(n); };

    var collect = function () {
      var L = [], vv = window.visualViewport;
      L.push('▼ ' + location.pathname + location.search);
      L.push('innerHeight=' + r0(window.innerHeight) +
             ' clientHeight=' + r0(document.documentElement.clientHeight) +
             ' scrollY=' + r0(window.scrollY));
      if (vv) {
        L.push('visualViewport h=' + r0(vv.height) + ' offsetTop=' + r0(vv.offsetTop) +
               ' pageTop=' + r0(vv.pageTop) + ' scale=' + (Math.round(vv.scale * 100) / 100));
      }
      L.push('standalone=' + standalone + ' dpr=' + window.devicePixelRatio);

      document.querySelectorAll('.rg-fab, .refresh-button').forEach(function (el) {
        var b = el.getBoundingClientRect(), s = getComputedStyle(el);
        L.push('[' + (el.className || '?') + ']');
        L.push('  下端との差=' + r0(window.innerHeight - b.bottom) +
               ' ← 16前後なら正常 / top=' + r0(b.top));
        L.push('  css bottom=' + s.bottom + ' position=' + s.position +
               ' style.bottom=' + (el.style.bottom || '(なし)'));
        var a = el.parentElement, culprit = 'なし(=viewport基準)';
        while (a && a !== document.documentElement) {
          var g = getComputedStyle(a);
          if (g.transform !== 'none' || g.filter !== 'none' || g.perspective !== 'none' ||
              (g.contain || '').indexOf('paint') >= 0 ||
              (g.willChange || '').indexOf('transform') >= 0 ||
              (g.backdropFilter && g.backdropFilter !== 'none')) {
            culprit = a.tagName + '.' + (a.className || '(無名)');
            break;
          }
          a = a.parentElement;
        }
        L.push('  包含ブロックを奪う祖先=' + culprit);
      });

      var h = getComputedStyle(document.documentElement), bd = getComputedStyle(document.body);
      L.push('html overflow-x=' + h.overflowX + ' / body overflow-x=' + bd.overflowX);
      L.push('scrollHeight=' + document.documentElement.scrollHeight);
      return L.join('\n');
    };

    var toggleDiag = function () {
      if (diagPanel) {
      ...
      diagPanel.setAttribute('style',
        'position:absolute; z-index:99999; left:8px; right:8px; top:' + (window.scrollY + 50) + 'px;' +
        'background:#fffefb; color:#2e2a24; border:2px solid #1f5c3d; border-radius:10px;' +
        'padding:10px; font:11px/1.5 ui-monospace,SFMono-Regular,monospace;' +
        'white-space:pre-wrap; word-break:break-all; box-shadow:0 6px 20px rgba(0,0,0,.35);');
      document.body.appendChild(diagPanel);
      var tick = function () { diagPanel.textContent = collect(); };
      tick();
      diagTimer = setInterval(tick, 400);
    };
    ...
    fabs.forEach(function (f) {
      ...
      f.addEventListener('touchstart', function () {
        fired = false;
        timer = setTimeout(function () { fired = true; toggleDiag(); }, 1200);
      }, { passive: true });
