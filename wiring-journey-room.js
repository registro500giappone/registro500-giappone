/* ストーリー第8回「ルームランプが点かない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   ...
   第7回（ホーン）と同じ F1 の負荷側にいる2つ＝両方が死んだらヒューズ1本で説明が付く。 */
(function () {
  'use strict';
  ...
  function draw(k, mode) {
    ...
    /* 赤地に白抜きの札＝この絵でいちばん見てほしい所に1枚だけ置く（第4・6・7回と同じ道具）。 */
    function chip(x, y, t) {
    ...
    /* 【赤の序列】切断はすき間13・×・ハロー・図幅いっぱいの境界破線・赤札1枚。このストーリーは縦軸が2本ある（幹線 X とドア柱 DX）ので、札の x も引数で受ける＝空いている側がどこかは切る場所ごとに違う（第6回は右・第7回は右・ここは中央寄り）。 */
    ...
    /* ===== バッテリー〜ヒューズ箱の電源側＝第7回とまったく同じ道 ===== */
    k.battery(X);
    /* 端子バッジ＝原典に番号のある端子にだけ付ける（第4回の作法／第6・7回で全号展開）。
    ...
    /* ===== ヒューズ F1＝第7回と同じ本線。ここから先の2本がホーンとルームランプ ===== */
    k.node(X, FZ);
    k.dashOut(X, FZ, 146);
    ...
    /* ヒューズの先で2手に分かれる＝もう1本はホーンへ（第7回のストーリー） */
    k.node(X, FZ + 62);
    ...
    /* 状態の語は箱の外・右（第7回と同じ作法＝箱の中に置くと光のにじみに埋もれる）。 */
