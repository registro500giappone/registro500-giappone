/* ストーリー第13回「ルームランプが点かない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   このストーリーだけの特徴＝【帰り道が2本ある】。これまでのストーリーはどれも1本の輪だった。
   ルームランプは電球の−から①ミラーの内蔵スイッチ②ドア柱スイッチの2本に分かれ、どちらか一方でもアースに落ちれば点く。
   だから「ドアを開けても点かないが、スイッチを入れれば点く」（またはその逆）という切り分けが、部品を外さずにできる。
   第12回（ホーン）と同じ F1 の負荷側にいる2つ＝両方が死んだらヒューズ1本で説明が付く。 */
(function () {
  'use strict';
  var WC = Journey.WC, C = Journey.C;
  var X = 76;                                   /* 幹線（バッテリー〜ヒューズ〜電球）の縦軸 */
  var DX = 232;                                 /* ドア柱スイッチ側の縦軸 */
  var FZ = 208;                                 /* ヒューズ F1 の箱の上端（下端子は +62） */
  var LB = { x: 16, y: 332, w: 174, h: 118 };   /* ルームランプの箱（電球＋内蔵スイッチ） */
  /* 高さ76＝状態の語（ドアは開いている／閉じている）を接点の下に十分離すため。 */
  ...
  function draw(k, mode) {
    ...
    /* 赤地に白抜きの札＝この絵でいちばん見てほしい所に1枚だけ置く（第4・6・12回と同じ道具）。 */
    function chip(x, y, t) {
    ...
    /* 【赤の序列】切断はすき間13・×・ハロー・図幅いっぱいの境界破線・赤札1枚。このストーリーは縦軸が2本ある（幹線 X とドア柱 DX）ので、札の x も引数で受ける＝空いている側がどこかは切る場所ごとに違う（第6回は右・第12回は右・ここは中央寄り）。 */
    ...
    /* ===== バッテリー〜ヒューズ箱の電源側＝第12回とまったく同じ道 ===== */
    k.battery(X);
    /* 端子バッジ＝原典に番号のある端子にだけ付ける（第4回の作法／第6・12回で全号展開）。
       この絵で付けるのは + / 30（レギュレータ）/ 30（ヒューズ箱の電源側）/ 電球の + ・−の5か所。
       内蔵スイッチ・ドア柱スイッチの接点には付けない＝原典に番号が無い。このストーリーでは +・−を hero にしていない＝絞り込みが【テスターを出す前のスイッチ操作】で進む号だから（測る端子を強調すると、この号のいちばんの主張とぶつかる）。 */
    k.term(X, 74, '+', 'l');
    ...
    /* ===== ヒューズ F1＝第12回と同じ本線。ここから先の2本がホーンとルームランプ ===== */
    k.node(X, FZ);
    k.dashOut(X, FZ, 146);
    ...
    /* ヒューズの先で2手に分かれる＝もう1本はホーンへ（第12回のストーリー）。
       ⭐図に書いてよい「第N回」は【前の回】だけ＝後の回を指すと予告になり、並べ替えで嘘になる。 */
    k.node(X, FZ + 62);
    k.dashOut(X, FZ + 62, 168);
    k.label(172, FZ + 58, 'ホーンへ', C.sub, null, 10);
    k.label(172, FZ + 71, '（第12回）', C.sub, null, 10);
    ...
    /* 電球＝丸とフィラメント。点灯時は光のにじみを敷く（点滅させない・のアイコンで置き換えない） */
    var BY = LB.y + 46;
    if (sc.lampOn) {
    ...
    /* ＋のバッジは箱の【中】に置く＝この部品は電球も内蔵スイッチも1つの箱に入っていて、端子が箱の外に出ていない（第6回のコイルのように箱の外へは出せない）。 */
    k.term(X, LB.y + 14, '+', 'l');
    ...
    /* 状態の語は箱の外・右（第12回と同じ作法＝箱の中に置くと光のにじみに埋もれる）。 */
    ...
    /* ===== 帰り道②＝電球の−→ドア柱スイッチ（横に分かれる） ===== 分岐点は電球の−＝箱の中。
       箱の右辺に端子を出して、そこから横へ引く。 */
    var MY = LB.y + 64;
    ...
    k.label(6, 662, '⬆ どちらの帰り道も、車体からバッテリーの − へ戻る（第5回）。', C.sub, null, 10.5);
  ...
  Journey.boot({
    ...
    /* ルームランプ・ドア柱スイッチには印を打たない＝wiring-layout.json はオーナーの実測値だけでできていて、こちらの推定値を1つも入れていない（この2部品はまだ実測が無い）。 */
    carmap: function () {
      return {
