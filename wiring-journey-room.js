/* ストーリー第8回「ルームランプが点かない」の絵。journey_id: room_lamp_dark（URLと対＝変えない）
   ...
   このストーリーだけの特徴＝【帰り道が2本ある】。
     これまでのストーリーはどれも1本の輪だった。ルームランプは電球の − から
...
(function () {
  ...
  /* ⚠️高さ76＝状態の語（ドアは開いている／閉じている）を接点の下に十分離すため。
       62 にしたら文字が接点の斜め線に串刺しにされた（第4・6回と同じ穴・実測して直した）。 */
  ...
  function draw(k, mode) {
    ...
    /* 【赤の序列】切断は すき間13・×・ハロー・図幅いっぱいの境界破線・赤札1枚（2026-08-24・第4回）。
       ⚠️このストーリーは縦軸が2本ある（幹線 X と ドア柱 DX）ので、札の x も引数で受ける
         ＝空いている側がどこかは切る場所ごとに違う（第6回は右・第7回は右・ここは中央寄り）。
       ⚠️切断を描く区間は 56px 以上空ける＝ハロー（r=19）が隣の記号と食い合う（第6・7回の実測）。 */
    ...
    /* ⚠️＋のバッジは箱の【中】に置く＝この部品は電球も内蔵スイッチも1つの箱に入っていて、
         端子が箱の外に出ていない（第6回のコイルのように箱の外へは出せない）。
       ⚠️電球の光のにじみ（r=30）に入るが、バッジは白い塗りを後から描くので読める（実測）。 */
    k.term(X, LB.y + 14, '+', 'l');
    ...
    /* 状態の語は箱の外・右（第7回と同じ作法＝箱の中に置くと光のにじみに埋もれる）。
       ⚠️高さは電球の真横（BY+5）ではなく箱の上寄り＝電球の − から右へ出る線のラベル
    ...
    /* ===== 帰り道②＝電球の − → ドア柱スイッチ（横に分かれる） =====
       ⚠️分岐点は電球の − ＝箱の中。箱の右辺に端子を出して、そこから横へ引く。 */
    var MY = LB.y + 64;
