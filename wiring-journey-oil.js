/* ストーリー第2回「油圧警告灯が消えない」の絵。journey_id: oil_lamp_on（URLと対＝変えない）
   共通の土台（場面の作り方・描画プリミティブ・共通部品・実車図・検算・起動）は /wiring-journey.js。
   ⚠️図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。

   第1回（チャージランプ）との関係:
     上半分（バッテリー→スターターレバー節点→レギュレータ30→キースイッチ→警告灯）は同じ道を通る＝
     共通部品と同じ座標で描いている。違うのは警告灯から下＝「落とし先」だけ。
     原典でも計器盤の4つの警告灯はどれも INTER←AZZURRO で受けていて、反対側の落とし先だけが違う
...
(function(){
  ...
  /* ---- トグル（唯一の操作＝停止中↔回転中） ---- */
  /* ⚠️キャプションは「いまこの絵で起きていること」だけを言う。しくみの説明（なぜ接点が
     開くのか等）は下の3段の箇条書きが持つ＝両方に書くと重複する（2026-08-21 FB） */
  var CAPS = {
