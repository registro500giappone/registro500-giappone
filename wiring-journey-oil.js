/* ストーリー第2回「油圧警告灯が消えない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   第1回（チャージランプ）との関係: 上半分（バッテリー→スターターレバー節点→レギュレータ30→キースイッチ→警告灯）は同じ道を通る＝共通部品と同じ座標で描いている。
   違うのは警告灯から下＝「落とし先」だけ。原典でも計器盤の4つの警告灯はどれも INTER←AZZURRO で受けていて、反対側の落とし先だけが違う（GENERAT.=51→レギュレータ／OLIO=0→油圧センダ→アース）。
   この構造がそのまま絵になる。 */
(function(){
  'use strict';
  ...
  function draw(k, mode){
    ...
    /* ===== ランプ→油圧センダ（w07-01 GRIGIO）。ここが第1回と分かれる所 ===== */
    if(mode.detach){
      /* 切り分けの手：センダの端子から線を外した状態。これは接点の位置ではなく「線が1本無い」状態＝netlist から w07-01 を外して solve している。
         外した線はメーター側（quadro.0）にぶら下がったままなので、色はその端子の状態で塗る（＝負荷の先＝12Vは来ているが流れない）。 */
    ...
    /* 「なぜ閉じているか」は電気では分からない（本当に圧が無い／固着／地絡）＝ここでは接点の事実と、しくみだけを言う */
  ...
  /* ---- トグル（唯一の操作＝停止中↔回転中） ---- */
  /* キャプションは「いまこの絵で起きていること」だけを言う。しくみの説明（なぜ接点が開くのか等）は下の3段の箇条書きが持つ＝両方に書くと重複する */
  var CAPS = {
    STOP:'<b>キーON・停止中＝点く。</b>黄色い点が一周しています。この輪がつながっている間、ランプは点きます。',
    RUN:'<b>エンジン回転中＝消える。</b>センダの接点が開いて、輪が切れました。黄色い点がどこにも無いのが「流れていない」印です。'
  };
  ...
  Journey.boot({
    ...
    scenes: function(scenario){
      return [
        /* 異常＝回転中なのにセンダの接点が閉じたまま。「本当に油圧が低い」も「センダの固着」も「灰色の線が車体に触れている」も、電気的には全部この同じ状態になる */
        {id:'j-fault', sc:scenario({inputs:{key:'ON',engine:'RUN'},override:{oil_sender:'LOW'}}), mode:{suspect:true}},
        /* 切り分け＝エンジンは止めたまま・キーONで、センダから線を1本外す。「外した」は接点の位置ではなく線が無い状態なので netlist から w07-01 を外して解く */
