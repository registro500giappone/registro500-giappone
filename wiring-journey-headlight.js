...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* ================= ⭐絵の中のスイッチを直接押せるようにする（2026-08-30・ユーザー要望） =================
       欄外のトグルと同じ操作を、絵の中の【そのスイッチ】の上でもできるようにする。
       ⭐透明な当たり判定の板をいちばん上に重ねるだけ＝絵の見た目は1ピクセルも変えない。
       ⚠️fill は "transparent"（"none" だとクリックが素通りする）。
       ⚠️最後に push する＝上に重ねないと、箱の中の文字や接触子がクリックを先に取る。
       ⚠️指で押せる大きさに広げてある（箱の中で隣と重ならない範囲いっぱい）。
       ⛔主図のときだけ描く＝紙芝居のコマに押せる板を置くと、押しても何も起きず読者を惑わす。 */
    if (m.main) {
      ...
      /* ライトスイッチの「切」「入」。⚠️座標は上の lightSwitch() と対（sx = bx+45+i*70） */
      hit(85, SW + 38, 60, 36, 'lights=OFF', 'ライトスイッチを切る');
      hit(155, SW + 38, 60, 36, 'lights=ON', 'ライトスイッチを入れる');
      /* コラムレバーの3段。⚠️端子の間隔が狭い（ハイ110・中央150・ロー216）ので、
         隣との中点で割って箱の幅いっぱいまで広げている。 */
      hit(68, CM + 52, 62, 36, 'beam=HIGH', 'レバーを III（ハイビーム）へ');
      hit(130, CM + 52, 53, 36, 'beam=POS', 'レバーを I（車幅灯）へ');
      hit(183, CM + 52, 53, 36, 'beam=LOW', 'レバーを II（ロービーム）へ');
    }
  ...
  /* ⭐トグルは2軸＝ダッシュのライトスイッチ（切／入）× コラムレバー（I／II／III）。
     ...
     ⚠️その「押しても変わらない」こそがこの回の主張（スイッチを2つ通らないと点かない）なので、
       ⛔黙って何も起きないままにせず、必ずキャプションで言葉にする。 */
  var CAPS_OFF = {
    POS: '<b>ライトスイッチが切。</b>レバーは I（車幅灯）の位置ですが、'
       + '<b>スイッチから先へは電気が出ていきません</b>。ヘッドライトまでの道が、どこも光っていないのが見えます。',
    LOW: '<b>ライトスイッチが切のまま、レバーをロー側へ倒しました。</b>——<b>何も変わりません。</b>'
       + '⭐この回路は<b>スイッチを2つ通らないと点かない</b>ので、<b>手前のひとつが切れている間、レバーがどこにあるかは関係がない</b>のです。',
    HIGH: '<b>ライトスイッチが切のまま、いちばん下（ハイ）まで倒しました。</b>——<b>やはり何も起きません。</b>'
       + '⭐犯人がライトスイッチより手前にいるとき、<b>レバーをいくら動かしても症状は1ミリも変わりません</b>。'
       + 'これが「レバーを動かして変化が無いなら、見に行くのはダッシュの裏」と言える理由です。'
  };
  var CAPS_ON = {
    POS: '<b>ライトスイッチを入れました。レバーはまだ I。</b>スイッチの箱は「入」に変わりましたが、'
       + '<b>黄色い点はヘッドライトへ進みません</b>——<b>レバーが電気をロー側にもハイ側にも渡していない</b>からです。'
       + '（車幅灯とナンバー灯はこの位置でもう点いています。この絵は F5・F6 の先を端子で止めてあるので、そこは描かれていません。）'
       + '⭐<b>車幅灯は点いているのに前だけ暗い</b>なら、犯人はレバーから先です。',
