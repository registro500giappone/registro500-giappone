# _top_stage.html を、外部取得なしで動く1枚の写しに焼く（artifact 公開用）
# 使い方: repo直下で  python <このファイル>
import io, os
src = io.open('_top_stage.html', encoding='utf-8').read()
assets = {
...
s = s.replace('<title>第2層の舞台・実配置版（上面図にカメラで寄る）</title>', '<title>500F 実配置ステージ</title>')
...
s = s.replace(tail,
    ...
    "  drawWires(); drawParts(); drawGrounds();\n"
    "\n  apply(VB);\n})();\n</script>")
...
s += ('\n<p class="note" style="margin-top:14px;border-top:1px solid var(--line);padding-top:10px">'
      'この頁はローカル作業ページ <code>_top_stage.html</code> の写しです。'
