...
#### 126 車種リスト（16種類）
> ⚠️ **【2026-08-10 追記】以下は実装当初の記録。現行の区分はこれではない。**現行の正本は `edit.html`。
> その後、生産国・世代・コーチビルドを含む1階層のリストに拡張された。
> **排気量をモデル名に含める方式（旧「Fiat 126 (594cc)」「(650cc)」）は廃止**。エンジンの載せ替えが多く、車体の世代と一致しないため。
> なお2026-08-10に「500と同じ2階層へ揃える」改修を検討したが、**日本国内の流通表記（日本語Wikipedia・旧車メディア）は時系列の1階層であり、
> 2階層化はかえって独自ルールになる**と判明したため撤回した。**126は1階層のまま維持すること。**
1. Fiat 126 (594cc)
2. Fiat 126 (650cc)
3. Fiat 126E
4. Fiat 126 DeLuxe
