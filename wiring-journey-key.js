...
(function () {
  ...
  function draw(k, mode) {
    ...
    k.node(X, 106);
    k.dashOut(X, 106, 190);
    label(184, 98, '始動レバー・セルへ', C.sub, null, 10.5);
    label(184, 111, '（第3号の旅）', C.sub, null, 10.5);
    ...
    else { seg(X, 106, X, 152, 'w11-03', true); label(X + 12, 134, 'MARRONE 茶・太', WC.MARRONE); }
    k.node(X, 152);
    ...
    else { seg(X, 152, X, 208, 'w11-04', true); label(X + 12, 184, 'ROSSO 赤・太', WC.ROSSO); }
    k.node(X, 208);
    ...
    label(182, 250, 'ホーン・ルームランプへ', C.sub, null, 10);
    label(182, 263, '（この旅は通らない）', C.sub, null, 10);
    ...
    /* ===== キースイッチ（30 ↔ 15/54） ===== */
    k.keySwitch(X, 262, pos.ign_sw === 'ON');

    /* ===== キー → 計器盤の INTER ===== */
    if (mode.cut === 'w10-02') cutSeg(310, 372, 'w10-02');
    else { seg(X, 310, X, 372, 'w10-02'); label(X + 12, 344, 'AZZURRO 水色', WC.AZZURRO); }
    ...
    /* ⚠️容疑の囲いは計器盤の手前で止める＝2つの球が同時に切れることは、まず無い。
       囲いを灯まで広げると「球も疑え」に読めて、絞り込みが1段ゆるくなる。 */
    if (mode.suspect) {
      k.suspect(4, 96, 174, 270, 92, '容疑者はこの中（セルが回るなら）');
      label(6, 600, '⚠️計器盤の中（レールと球）が、囲いの外に残る最後の容疑者', C.sub, null, 10.5);
      return 614;
    }
    if (mode.minus) {
      label(6, 600, '⚠️線には12Vが来ているのに、灯は1つも点かない。', C.hi, null, 10.5);
  ...
  var CAPS = {
    ...
    ON: '<b>キーON＝2つとも点く。</b>黄色い点が、バッテリーから車の後ろのレギュレータを回って前へ戻り、キーを通って計器盤まで来ています。灯が点くのは「そこまで電気が届いた」という合図です。'
  };
  ...
  var CHECKS = [
    ...
    { label: '灰色線（w07-01）だけ外れた・キーON→油圧灯', s: { inputs: ON, ops: cut('w07-01') }, expect: false, words: LW },
