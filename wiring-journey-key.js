/* ストーリー第4回「キーを回しても何も点かない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   第3回（セルが回らない）との関係＝このストーリーは【第3回の対】: 第3回…警告灯は点くのにセルが回らない→容疑者はスターターレバーの節点より【下】第4回…警告灯が点かない→容疑者は節点より【上】か、節点から計器盤までの1本道どちらの絵にも「スターターレバーの節点」が出てくる。
   そこが電気の分かれ道だから。このストーリーだけの特徴＝【主役が1つの警告灯ではなく「2つの警告灯が揃って消えていること」】。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* 赤地に白抜きの札＝この絵でいちばん見てほしい所に1枚だけ置く。札は1つの絵に1枚まで。
       2枚置くと、どちらを見ればいいのか分からなくなる。 */
    ...
    /* 線が1本外れている場面の描き方（第3回と同じ作法）。これは接点の位置ではなく「線が無い」状態＝netlist から外して解いている。
       【＝この絵でいちばん強い赤にした。すき間を広げ・×を打ち・札を添え、さらに図幅いっぱいの境界線で【ここから下は電気が来ていない】ことを一目で言う。
    ...
    /* ===== ＋の太線→スターターレバーの節点。ここで第3回のストーリー（セル）と分かれる ===== 端子バッジは【原典に番号がある端子】にだけ付ける。
       この線が出るのはバッテリーの＋端子なので、ここも同じ形で名乗らせる。右へ出した分、色ラベルは 88→94 へ下げてバッジの下に逃がしてある。 */
    seg(X, 62, X, 106, 'w11-01', true);
    k.term(X, 74, '+', 'r');
    label(X + 12, 94, 'ROSSO 赤', WC.ROSSO);
    k.node(X, 106);
    /* ここは端子ではない＝原典に番号が無い。バッジを付けると実車に無い端子を探させる */
    /* y=110/122＝囲いのラベル（y=90）とレギュレータの箱（y=130〜）の隙間。 */
    label(X - 14, 110, '分かれ道', C.sub, 'end', 9.5);
    label(X - 14, 122, '（端子ではない）', C.sub, 'end', 9.5);
    /* この枝ラベルは容疑の囲い（x=4..192）の【外】に置く＝中に入ると「セルも容疑者」に読める。
       囲いを右へ広げた分、開始 x も 184→198 へ逃がした。 */
    k.dashOut(X, 106, 194);
    label(198, 98, 'セルモーターへ', C.sub, null, 10.5);
    label(198, 111, '（第3回のストーリー）', C.sub, null, 10.5);

    /* ===== 節点→レギュレータの端子30（車の後ろ）===== ここが 500 の意外なところ＝キーに来る電気は、いったん車の【後ろ】まで行って前へ戻ってくる。
       原典の 30 系はレギュレータの端子で1つに集まっている。 */
    if (mode.cut === 'w11-03') cutAt(X, 106, 152, 'w11-03', true, { line: true });
    /* 色ラベルは容疑の囲い（右端192）の内側に収める＝破線に文字を貫かれる。 */
    else { seg(X, 106, X, 152, 'w11-03', true); label(X + 12, 134, 'MARRONE 茶', WC.MARRONE, null, 10.5); }
    k.node(X, 152);
    /* 端子名は箱の【外】に置く＝濃い箱の上に濃い字を置くと沈んで読めない（第3回の教訓）。
    ...
    /* 入る線（茶）と出る線（赤）は【同じ 30 端子】に着いている＝原典どおり。バッジを1つだけ置いて、そこへ上下から線が刺さっている絵にする（2つ置くと別端子に見える）。 */
    k.term(X, 152, '30', 'r');
    ...
    k.node(X, 208);
    /* ヒューズ F1＝幹線にぶら下がる枝。キースイッチへの赤線は【ヒューズの電源側】から出る＝F1が切れても警告灯は点く（第1回で先回りして潰したのと同じ迷い道）。 */
    ...
    /* ===== キースイッチ（30 ↔ 15/54）＝このストーリーの主役 ===== 部品名は x=198 から＝容疑の囲い（右端192）の外。
       図に無い端子を本文が「当てろ」と指していたのが、やり直し前のいちばんの穴だった。 */
    k.keySwitch(X, 262, pos.ign_sw === 'ON', 198, { hero: true });
    /* バッジは箱の【外】＝y=250/322。hero のハロー（y=255〜317）に掛けると箱に食い込んで見える。 */
    ...
    /* ===== 計器盤の中＝1本の給電レール（INTER）が2つの警告灯を養っている ===== 計器盤の内部結線は原典の図から読み切れていない（v:inf）。
       だから細い内部線として描き、電線の被覆色は付けない＝「原典に書いてあること」と見分けが付くようにする。 */
    s.push('<rect x="12" y="372" width="276" height="140" rx="10" fill="' + C.body + '" stroke="' + C.deep + '" stroke-width="2.5"/>');
    /* 見出しは短く＝x=100 の内部線を横切らない長さに収める（12文字だと線に串刺しになった） */
    s.push('<text x="20" y="388" font-size="10.5" fill="' + C.in_ + '">計器盤の中</text>');
    k.node(X, 372);
    /* 箱の【中】に置く＝箱の上（y=364）だと容疑の囲いの下辺（y=366）を跨ぐ */
    ...
    /* 2つまとめて言う＝このストーリーの主役は「1つの警告灯」ではなく「2つが揃っているか」。
       点灯時のにじみは中心 y=432・ry=56＝下端 488 まで広がる。状態語はその外（y=496）に置く（488 より上に置くと赤い光に埋もれて読めなくなった。 */
    ...
    /* 警告灯から下＝落とし先。ここから先はそれぞれ別のストーリーなので、色だけ見せて手放す。
       端子名（51・0）は箱の中なので明るい色で書く＝濃い地に濃い字にしない */
    ...
    /* 落とし先の線が外れる場面（片方だけ消える）も【切れているように描く】。線が薄くなるだけでは「外れている」と読めない＝やり直し前はここが無言だった。
    ...
    label(6, 582, '緑→レギュレータ51（第1回）／灰→油圧センダ（第2回）', C.sub, null, 10.5);

    /* 容疑の囲いは計器盤の手前で止める＝2つの電球が同時に切れることは、まず無い。囲いを警告灯まで広げると「電球も疑え」に読めて、絞り込みが1段ゆるくなる。
       条件のほうは本文（cap）が「①でセルが回った場合」と言う。 */
    if (mode.suspect) {
      /* quiet＝囲いを一段引く。同じ絵にある「切れている場所」の赤と争わせない */
      k.suspect(4, 96, 188, 270, 90, '容疑者はこの中', true);
      label(6, 600, '⚠️計器盤の中（給電レールと電球）が最後の容疑者', C.sub, null, 10.5);
      return 614;
    }
    if (mode.minus) {
      /* この場面だけ、外れている所が【線の上に無い】＝どの線も生きて見えるので、印が無いと「どこも壊れていない絵」に読めてしまう（やり直し前がこれ）。
         バッテリーの−端子そのものに印を打つ。帰り道は原典の図でも車体を通る＝線として描けないので、札で言葉にする。 */
      /* ハローだけ r=15（他の切断点は19）＝端子が図の上端 y=0 の近くにあり、19だと上が切れる */
      var mx = X + 27.5, my = 15;
      ...
      s.push('<path d="M' + (mx - 5.5) + ',' + (my - 5.5) + ' L' + (mx + 5.5) + ',' + (my + 5.5)
        ...
        + '" stroke="' + C.hi + '" stroke-width="3" stroke-linecap="round"/>');
      chip(168, my, '帰り道が外れている');   
      /* 「12Vは来ている」は言葉より当てて見せる＝テスターを給電レールに当てた絵にする */
      k.probe(X, 372, '12V', -36, -20);
      label(6, 600, '⚠️どの線にも色が付いたまま＝12Vはここまで来ている。', C.hi, null, 10.5);
      label(6, 616, 'それでも点かない。帰り道（バッテリーの −）が無い（→第5回）。', C.hi, null, 10.5);
      return 628;
    }
    if (mode.cutLow) {
  ...
  /* ---- トグル（唯一の操作＝キーOFF ↔キーON） ---- キャプションは「いまこの絵で起きていること」だけ。
     しくみは下の箇条書きが持つ。 */
  var CAPS = {
  ...
  var CHECKS = [
    ...
    /* −側が外れた＝帰り道が無い。で初めて正しく答えられるようになった場面。この2件は以前は「点く・回る」と誤答していた。 */
  ...
  Journey.boot({
    ...
    scenes: function (scenario) {
      return [
        ...
        /* 異常②＝帰り道（アース）が切れた。絵じゅうに色は付いたままで、警告灯だけが消える＝「12Vは来ているのに動かない」の見本。
           案Aを入れて初めて正しく描けるようになった場面。 */
        { id: 'j-minus', sc: scenario({ inputs: ON, ops: cut('w11-10') }), mode: { minus: true } },
        /* 片方だけ消える＝共通部分は無実。この1枚が絞り込みの根拠 mode.cutLow＝外した線を【切れている絵】で見せる（やり直し前は薄くなるだけだった） */
        { id: 'j-one', sc: scenario({ inputs: ON, ops: cut('w07-01') }), mode: { cutLow: 'w07-01' } },
        { id: 'j-fixed', sc: scenario({ inputs: ON }), mode: {} }
      ];
    },
    /* このストーリーも車の全長が要る＝キーに来る電気がいったん車の後ろまで行って戻るため。
       第3回と同じ scale 4（viewBox が広いので印も文字も拡大しないと読めない）。 */
    carmap: function () {
      return {
