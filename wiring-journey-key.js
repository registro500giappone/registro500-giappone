/* ストーリー第4回「キーを回しても何も点かない」の絵。journey_id: key_no_power（URLと対＝変えない）
   共通の土台（場面の作り方・描画プリミティブ・共通部品・実車図・検算・起動）は /wiring-journey.js。
   ⚠️図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。

   第3回（セルが回らない）との関係＝このストーリーは【第3回の対】:
     第3回 … 警告灯は点くのにセルが回らない → 容疑者はスターターレバーの節点より【下】
     第4回 … 警告灯が点かない              → 容疑者は節点より【上】か、節点から計器盤までの1本道
     どちらの絵にも「スターターレバーの節点」が出てくる。そこが電気の分かれ道だから。
  ...
  function draw(k, mode) {
    ...
    function cutAt(cx, y1, y2, id, thick, opt) {
      ...
      /* ⚠️札は図の【左の余白】に置く＝切断点の右（x=196〜）はヒューズ箱と枝ラベルで埋まっていて、
         そこへ出すと囲いの破線をまたぐか図の右端を越える（第4回で前に直したのと同じ型の事故）。
    ...
    /* ===== ＋の太線 → スターターレバーの節点。ここで第3回のストーリー（セル）と分かれる =====
    ...
    k.node(X, 152);
    /* ⚠️端子名は箱の【外】に置く＝濃い箱の上に濃い字を置くと沈んで読めない（第3回の教訓）。
    ...
    /* ヒューズ F1＝幹線にぶら下がる枝。⚠️キースイッチへの赤線は【ヒューズの電源側】から出る
       ＝F1が切れても警告灯は点く（第1回で先回りして潰したのと同じ迷い道）。 */
