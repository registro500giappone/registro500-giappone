...
--
-- 状態: ✅適用済み（2026-08-10 適用）
--   ※ 2026-08-12 に実測で確認。`note_prompt` が入っている項目は **30件**（`equipment_items` 総数74）。
--     ヘッダの「28」は起案時の数で、最終的に30項目になった（ユーザーは 2026-08-11 に実機で
--     30項目を確認し「煩わしくない・このまま維持」と判断済み）。
--   ⚠️ この1本だけ `apply_migration` を通しておらず、適用済みmigration一覧に名前が無い
--     （直接SQLで流したため）。**一覧に無い＝未適用、と読まないこと**。
-- ⛔ 再実行しないこと（update のみなので破壊的ではないが、以後の手直しを巻き戻す）。
--
-- 【背景】
