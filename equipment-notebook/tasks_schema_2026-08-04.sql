...
-- 状態: ✅適用済み（2026-08-05 適用）
-- 根拠: 2026-08-09 にローカルPCから実測。equipment_tasks 25行 / equipment_task_items 57行が存在し、
--       匿名（公開キー）でも両テーブルとも HTTP 200。※2026-08-06 にクラウドから「テーブルが存在しない
--       （PostgREST 400）」と判定したのは誤り。当時の判定手順に問題があったとみられる（再実行しないこと）。
--
