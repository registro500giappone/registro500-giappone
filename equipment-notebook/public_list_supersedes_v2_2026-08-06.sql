...
-- migration名: equipment_notebook_public_list_supersedes_v2_2026_08_06
-- 状態: ✅適用済み（2026-08-06 起案 ／ 2026-08-06 本番データで適用を実地確認）
-- 根拠: スシオさん(id66車載)がマエストロ判定=true＝id71が充足扱いになっている
--        ※ v1（supersedes初版）は適用済み。これはポイント・コンデンサーを足した差し替え版。
--          create or replace なので、これ1本を流せば最新状態になる。
--
...
--     67 デスビ本体       → 19 デスビキャップ / 20 ローター / 15 ポイント / 16 コンデンサー
--
--   ※ ポイント(15)・コンデンサー(16)も含める（2026-08-06 ユーザー判断）。
--     本体が丸ごとあれば、これらの予備を車載していなくても載せ替えで復旧できるため。
--     判定が見ているのは「対応力」なので、部品単位でなく復旧できるかで揃える。
