...
- **不具合B: SoftBank等キャリアメールでOTPコードが読めない（テンプレート簡素化で対応・要ダッシュボード反映）**。`napdesign@softbank.ne.jp` 宛のOTPメールが、キャリアのメールゲートウェイでHTML本文が壊れ「Attachment.html（723バイト、開けない）」に変換されていた（元テンプレは3244バイト・テーブルレイアウト＋背景色＋letter-spacing等の装飾あり）。`py/supabase_otp_email_template.html` を1412バイトまで簡素化（table/背景色/box-shadow等を除去、`<p>`のみの構成）。**過去に別の理由（style属性のシングルクォートでSupabaseのGoテンプレートパーサーが500エラーを起こした事故＝2026-06-26復旧）があるため、シングルクォート・HTMLコメント・rgba/box-shadowを使わない制約を今回も厳守して作成**。**このファイルはリポジトリ内の控えのみで、実際に使われるにはSupabaseダッシュボード（Authentication > Email Templates）への手動貼付が必要（ユーザー作業）**。
...
- 検証：`equipment-edit.html`・`edit.html` とも構文チェックOK。テンプレートは既知の危険パターン3点（シングルクォートCSS・コメント・rgba/box-shadow）を回避していることを目視確認。**ログイン後の実データでの通し確認は依然未実施**。
