/* 旅ページ第3号「セルが回らない」の絵。journey_id: starter_no_crank（URLと対＝変えない）
   ...
   第1号（チャージランプ）・第2号（油圧警告灯）との関係:
     ...
     この旅は【主役が警告灯ではない】＝負荷はスターター（モーター）。バッテリーの＋端子から
     始動レバーの節点を経て、太い線1本でセルへ落ちる短い輪で、灯の道とはまったく別物。
     節点から右へ分かれるのが前2つの旅＝この絵では「灯へ行く枝」として同じ画面に出す。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* ===== 節点＝始動レバーの入口。ここで旅が2つに分かれる ===== */
    k.node(X, 104);

    /* 右の枝＝第1号・第2号の旅（レギュレータ30→F1→キースイッチ→計器盤）。
    ...
    s.push('<text x="100" y="132" font-size="12" fill="#fffdf8" text-anchor="middle">始動レバー</text>');
    s.push('<text x="100" y="147" font-size="10.5" fill="' + C.in_ + '" text-anchor="middle">（運転席の引きノブ）</text>');
  ...
  var CELLW = ['回る', '回らない'];   /* ⚠️この旅の主役はモーター＝共通ランタイムの既定語（点く/消える）は使えない */
  var LAMPW = ['点く', '消える'];
  var CHECKS = [
  ...
  Journey.boot({
    ...
    /* 右の枝の灯（＝第1号・第2号の旅の主役）を、この旅でも読めるようにする */
    ...
    /* ⚠️この旅だけ車の全長が要る＝バッテリーは車の前・セルは後ろ。第1・2号のエンジンルーム
