/* ストーリー第3回「セルが回らない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   第1回（チャージランプ）・第2回（油圧警告灯）との関係: 前2つは「計器盤の警告灯」が主役で、どちらもキースイッチ→計器盤の道を通っていた。
   このストーリーは【主役が警告灯ではない】＝負荷はスターター（モーター）。バッテリーの＋端子からスターターレバーの節点を経て、太い線1本でセルへ落ちる短い輪で、警告灯の道とはまったく別物。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* ===== ＋の太線（w11-01 ROSSO太）→スターターレバーの節点 ===== */
    if (mode.cut === 'w11-01') {
      /* ＋端子から外れている＝線はレバー側にぶら下がったまま。これは接点の位置ではなく「線が1本無い」状態＝netlist から w11-01 を外して解いている。 */
    ...
    /* ===== 節点＝スターターレバーの入口。ここでストーリーが2つに分かれる ===== */
    k.node(X, 104);

    /* 右の枝＝第1回・第2回のストーリー（レギュレータ30→F1→キースイッチ→計器盤）。
       途中は省略するが、警告灯が点いているかどうかは solve() の答えをそのまま出す＝「警告灯は点いているのにセルは回らない」が同じ1枚で言える。 */
    ...
    /* レバーの絵（引くと手前＝左へ出る）。位置は controls の starter_sw から＝勝手な演出ではない */
    ...
    /* ===== セルモーター＝このストーリーの主役の負荷。呼び名は「セルモーター」で統一（旧「スターター」表記は本文と食い違うので廃止。
       本文では略して「セル」と書く） ===== */
    ...
    /* ピニオンとフライホイールの歯。噛むかどうかは電気ではなくレバー（機械）で決まる＝「ガチャッと言うのに回らない」が起きる理由がここに見える。
       部品の箱は x=34..166。ピニオンとそのラベルは必ず箱の右外（>166）に置く（箱にかかると濃い地に濃い字で沈んで読めなくなる。 */
    var px = pull ? 192 : 181;
    ...
    /* 容疑の囲いは警告灯の小窓（x=183 から光る）に絶対にかけない＝警告灯は「無実の証人」で、囲いの中に入れると意味が反転する。
       右端は 178 まで。 */
    if (mode.suspect) {
  ...
  /* ---- トグル（唯一の操作＝レバーを戻す↔引く） ---- キャプションは「いまこの絵で起きていること」だけを言う。
     しくみ（なぜキーでは回らないか等）は下の3段の箇条書きが持つ＝両方に書くと重複する */
  var CAPS = {
  ...
  var CELLW = ['回る', '回らない'];   /* このストーリーの主役はモーター＝共通ランタイムの既定語（点く/消える）は使えない */
  var LAMPW = ['点く', '消える'];
  var CHECKS = [
    ...
    /* 【。案A（アースの基準をバッテリー−へ移す）を入れるまで、この2件は「回る・点く」と誤答していた＝限界②として文章で断っていたもの。
  ...
  Journey.boot({
    lampId: 'starter',                          /* 主役は警告灯ではなくモーター */
    lampName: 'セル',
    alt: false,
    mainInit: 'OFF',
    mainInputs: function (v) { return { key: 'ON', engine: 'STOP', starter: v }; },
    /* 右の枝の警告灯（＝第1回・第2回のストーリーの主役）を、このストーリーでも読めるようにする */
    ...
    /* このストーリーだけ車の全長が要る＝バッテリーは車の前・セルは後ろ。第1・2回のエンジンルーム（665幅）に対して 3020 幅＝4.5倍広いので、印も凡例も拡大しないと読めない。
       縦は車体の外まで余白を取る（y -140〜1500）＝車の輪郭で切らず、凡例を車に重ねない。
       ラベルの向きは左右の衝突で決める：バッテリーは右へ、後ろの2つは左へ出す。 */
    carmap: function () {
      return {
        viewBox: '0 -140 3020 1640',
        scale: 4,
        marks: [{ id: 'battery', color: '#b8442e', label: 'バッテリー', anchor: 'start' },
                /* この●は wiring-layout.json の実測値＝【運転席のスターターレバー】の位置。
