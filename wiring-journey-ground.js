/* 旅ページ第5号「あちこち同時に点かない（アース）」の絵。journey_id: ground_return（URLと対＝変えない）
   共通の土台（場面の作り方・描画プリミティブ・共通部品・実車図・検算・起動）は /wiring-journey.js。
   ⚠️図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。

   この旅だけの特徴＝【絵の向きが裏返っている】。
     第1〜4号 … ＋の道（行き）を上から下へたどる。アースは絵の一番下の記号。
     第5号   … その【帰り道】だけを描く。＋の道は破線1本に畳んで「第1〜4号でたどった」と手放す。
   だから幹線は赤ではなく車体（ボディ）で、黄点は下から上（バッテリーの − へ戻る向き）に流れる。

   ⚠️この旅は 2026-08-21 の「案A」（アースの基準を車体からバッテリーの − へ移す・wiring-sim.js）が
     入って初めて成立する。案Aが無いと車体が無条件のアース源になり、−端子が外れても
     「点く・回る」と誤答した＝旅の山場（j-minus）がそもそも描けない。
   ⚠️データ追加ゼロ＝既存のアース線5本（w11-08/09/10/11・w07-02）だけで全場面が解けている。 */
(function () {
  'use strict';
  var WC = Journey.WC, C = Journey.C;
  /* ⚠️箱は左に寄せてある＝アース線（横）を長く取るため。短いと黄点が2つしか乗らず、
     「流れている」が伝わらない（実測して 46→68 に広げた）。 */
  var BX = 26, BW = 142, BR = BX + BW;     /* 部品の箱 */
  var XB = 250, BDX = 236, BDW = 28;       /* 車体（ボディ）の板＝この旅の幹線 */
  var BDY = 100, BDH = 290;
  ...
  function draw(k, mode) {
    ...
    /* 縦の線を1本外して描く（第3・4号と同じ作法）。⚠️接点ではなく「線が無い」状態 */
    ...
    k.label(6, 96, '＋（行き）の道は第1〜4号でたどった', C.sub, null, 10);
    ...
    /* 板の名前は【板の右外】に縦書きで置く＝板の中に置くと黄点の列と重なる（第3号の教訓と同じ、
       濃い所に文字を乗せない）。x=268〜300 の余白に収める。 */
    ...
    ROWS.forEach(function (r) {
      ...
      /* 状態の語＝濃い箱の上なので明るい色で書く（第3号の教訓）。キーOFFは異常ではないので中立色 */
    ...
    /* ===== 板の下＝行き止まり。ここに何も無いことが、この旅の言いたいこと ===== */
  ...
  /* ---- トグル（キーON ↔ 始動レバーを引く） ----
     ⚠️どちらも「正常な操作」。この旅は故障を場面（下の絵）で見せるので、トグルは
    ...
    carmap: function () {
      return {
        ...
        legend: '<text x="40" y="1430" font-size="96" fill="#a49b87">←車の前方（トランク）</text>' +
                ...
                /* ⚠️viewBox の下端は y=1500＝ここより下に文字を置くと切れる（実測で確認）。
