/* ストーリー第5号「あちこち同時に点かない（アース）」の絵。journey_id: ground_return（URLと対＝変えない）
   ...
   ⚠️このストーリーは 2026-08-21 の「案A」（アースの基準を車体からバッテリーの − へ移す・wiring-sim.js）が
     入って初めて成立する。案Aが無いと車体が無条件のアース源になり、−端子が外れても
     「点く・回る」と誤答した＝ストーリーの山場（j-minus）がそもそも描けない。
    ...
    scenes: function (scenario) {
      return [
        ...
        /* ★③−端子が外れる＝全部死ぬ。しかも車体には12Vが回ってくる（post）。
           案Aを入れて初めて正しく描けるようになった場面＝このストーリーの山場 */
