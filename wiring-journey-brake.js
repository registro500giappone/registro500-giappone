/* ストーリー第9号「ブレーキランプが点かない」の絵。journey_id: brake_no_light（URLと対＝変えない）
   ...
   このストーリーだけの特徴が3つある。
     ...
     ②【ランプが2つあって、片方からもう片方へ渡る】＝原典の実配線が
        「SW→左マージンを降りて左後灯→車体の底を横断→右後灯」。左のランプのソケットのところで分かれるので、
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* 断線の印。⚠️絵の下半分（ランプのまわり）は文字の置き場が無い＝○印だけを打ち、
         「どこが外れているか」は絵の下の注記行が言う。
       ⚠️実測で分かったこと：左のランプのアースに『ここが外れている』を end で添えると
         図の左端からはみ出し、右へ回すとランプの状態語（点いている／点かない）と重なる。
    ...
    /* 横線（左のランプ→右のランプの渡り）の断線。○印だけ＝この高さには「ROSSO 赤」と
       「車体の底を横断」のラベルが既にいる。 */
    ...
    /* ⭐端子バッジ＝原典に番号のある端子にだけ付ける（第4号の作法／第6〜8号で全号展開）。
       ...
       ⛔ブレーキランプの口金には付けない＝実車の口金に「＋」と刻まれた端子は無く、
