/* ストーリー第9号「ブレーキランプが点かない」の絵。journey_id: brake_no_light（URLと対＝変えない）
   ...
   このストーリーだけの特徴が3つある。
     ①⭐【キーONのときだけ生きるヒューズ F2】がシリーズで初めて本線に立つ。
        F1（常時30系）のストーリー＝第7・8号と対になる形で、「キーを回さないと点かない」が絵に出る。
   ...
   ⚠️系統6 を新設したストーリー＝NET_VERSION 7→8
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* ⭐ヒューズ F1 は【このストーリーの本線ではない】＝常時30系の枝。第7・8号のストーリーがこの先にある。 */
    k.node(X, 186);
    k.dashOut(X, 186, 168);
  ...
  Journey.boot({
    ...
    /* ⛔ストップスイッチと後コンビランプには印を打たない＝wiring-layout.json はオーナーの実測値
       ...
       ⚠️代わりに車の全長を出す＝このストーリーは「ダッシュのキー → 前のヒューズ箱 → 車の最後尾」まで
