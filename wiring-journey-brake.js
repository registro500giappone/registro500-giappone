/* ストーリー第9回「ブレーキランプが点かない」の絵。journey_id: brake_no_light（URLと対＝変えない）
...
(function () {
  ...
  /* ⚠️高さ76＝箱の中の3要素（部品名・はたらき・接点の状態）を縦にずらして置くため。
       第8回のドア柱スイッチで 62 にしたら3件が同じ高さに重なった（実測して直した）。 */
  ...
  function draw(k, mode) {
    ...
    /* 後コンビランプの中のブレーキランプ。点灯時は赤いにじみを敷く（⛔点滅させない）。
       ⚠️にじみの半径は箱をわずかに超える程度まで＝大きくすると右の状態語を飲む（第8回の実測）。 */
    function stopLamp(cx, name, lit, cut) {
      ...
      /* ⚠️そのランプのアースを切る場面だけ、状態語を右へ逃がす＝×印（r=11）に触れるため。
         💡第4回 key.js の `label(LO + (mode.cutLow ? 26 : 10), ...)` と同じ型＝
    ...
    /* 状態の語は箱の外・右（第7・8回と同じ作法＝箱の中に置くと内部の線と重なる）。
    ...
    /* ⚠️注記は必ず2行に割る＝図の幅は 300 しかなく、font 10.5 の日本語は【1行22文字】で
         右端に届く。第8回までは1行で足りていたが、このストーリーは言うことが多い（実測ではみ出した）。 */
  ...
  Journey.boot({
    ...
    /* ⛔ストップスイッチと後コンビランプには印を打たない＝wiring-layout.json はオーナーの実測値
       ...
       ⚠️代わりに車の全長を出す＝このストーリーは「ダッシュのキー → 前のヒューズ箱 → 車の最後尾」まで
         行く長い旅なので、第3回（セル）と同じ広さの viewBox が要る。 */
    carmap: function () {
      return {
