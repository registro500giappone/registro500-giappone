/* ストーリー第7回「ブレーキランプが点かない」の絵。図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。
   ...
   F1（常時30系）のストーリー＝ホーン・ルームランプの回と対になる形で、「キーを回さないと点かない」が絵に出る。
...
(function () {
  ...
  function draw(k, mode) {
    ...
    /* 断線の印。絵の下半分（ランプのまわり）は文字の置き場が無い＝○印だけを打ち、「どこが外れているか」は絵の下の注記行が言う。
       ホーン・ルームランプの回と同じ病気（箱の近くには文字の逃げ場が無い）。 */
    ...
    /* ヒューズ F1 は【このストーリーの本線ではない】＝常時30系の枝。ホーン・ルームランプの回がこの先にある。
       ⛔図に回数を書かない＝連載の並びを組み替えるたびに嘘になる（2026-09-03 に、書いてあった回数が実際に古くなった）。 */
    k.node(X, 186);
    k.dashOut(X, 186, 168);
    k.label(172, 179, 'ヒューズ F1 →ホーン・', C.sub, null, 10);
    k.label(172, 192, 'ルームランプへ', C.sub, null, 10);
    ...
    /* 状態の語は箱の外・右（ホーン・ルームランプの回と同じ作法＝箱の中に置くと内部の線と重なる）。「固着」の場面では【ペダルを離しているのに閉じている】＝そう書かないと絵が嘘になる。 */
