/* 旅ページ第9号「ブレーキランプが点かない」の絵。journey_id: brake_no_light（URLと対＝変えない）
   共通の土台（場面の作り方・描画プリミティブ・共通部品・実車図・検算・起動）は /wiring-journey.js。
   ⚠️図の点灯・色・黄点はすべて wiring-sim.js（L1到達性）の solve() 結果＝絵に合わせて数字を作らない。

   この旅だけの特徴が3つある。
     ①⭐【キーONのときだけ生きるヒューズ F2】がシリーズで初めて本線に立つ。
        F1（常時30系）の旅＝第7・8号と対になる形で、「キーを回さないと点かない」が絵に出る。
     ②【灯が2つあって、片方からもう片方へ渡る】＝原典の実配線が
        「SW→左マージンを降りて左後灯→車体の底を横断→右後灯」。左の球の ＋ から右へ分かれるので、
        「右だけ点かない」と「左右とも点かない」で容疑者の範囲がはっきり分かれる。
     ③【症状が2つある】＝点かない／点きっぱなし。後者はスイッチの固着で、絵の上でも作れる。

   ⚠️系統6 を新設した旅＝NET_VERSION 7→8
     （部品 f2・brake_sw・stop_l・stop_r／電線 w06-01〜w06-06）。 */
(function () {
  'use strict';
  var WC = Journey.WC, C = Journey.C;
  var X = 76;                                   /* 幹線（バッテリー〜キー〜F2〜SW〜左灯）の縦軸 */
  var RX = 196;                                 /* 右のストップランプの縦軸 */
  var KT = 232;                                 /* キースイッチの箱の上端（高さ48） */
  var FZ = 330;                                 /* ヒューズ F2 の箱の上端（下端子は +62） */
  var SB = { x: 16, y: 440, w: 174, h: 76 };    /* ストップスイッチの箱 */
  /* ⚠️高さ76＝箱の中の3要素（部品名・はたらき・接点の状態）を縦にずらして置くため。
       第8号のドア柱スイッチで 62 にしたら3件が同じ高さに重なった（実測して直した）。 */
  var CR = 558;                                 /* 左灯→右灯へ渡る横線の高さ */
  var LT = 574, LH = 50;                        /* ストップランプの箱の上端と高さ */
  var GY = 668;                                 /* アース記号の高さ */
  ...
  function draw(k, mode) {
    ...
    k.node(X, 100);
    k.dashOut(X, 100, 190);
    k.label(184, 93, '始動レバー・セルへ', C.sub, null, 10.5);
    k.label(184, 106, '（第3号）', C.sub, null, 10.5);
    ...
    /* ⭐ヒューズ F1 は【この旅の本線ではない】＝常時30系の枝。第7・8号の旅がこの先にある。 */
    k.node(X, 186);
    k.dashOut(X, 186, 168);
    k.label(172, 179, 'ヒューズ F1 →ホーン・', C.sub, null, 10);
    k.label(172, 192, 'ルームランプ（第7・8号）', C.sub, null, 10);
    ...
    /* 状態の語は箱の外・右（第7・8号と同じ作法＝箱の中に置くと内部の線と重なる）。
       ⚠️「固着」の場面では【ペダルを離しているのに閉じている】＝そう書かないと絵が嘘になる。 */
    ...
    /* ⚠️このラベルは1つ上の「ROSSO 赤」（y=534）と縦に15px離す＝同じ帯に置くと重なる（実測）。
       ⚠️渡りを切った場面では出さない＝○印と重なる（実測して消した）。 */
    ...
    /* ⚠️注記は必ず2行に割る＝図の幅は 300 しかなく、font 10.5 の日本語は【1行22文字】で
         右端に届く。第8号までは1行で足りていたが、この旅は言うことが多い（実測ではみ出した）。 */
  ...
  /* ---- トグル（ブレーキ 離す ↔ 踏む）＝キーはONで固定 ----
     ⭐主役の動き＝「踏むと点く」。キーOFFの場面は下の「異常のとき」で別に出す。 */
  var CAPS = {
    ...
    PRESSED: '<b>ブレーキを踏んだ＝配管の油圧がスイッチの接点を閉じました。</b>足が接点を押しているのではありません。電気は赤い線で車の左後ろまで下って<b>左のストップランプ</b>を点け、そこから<b>車体の底を横断して右のランプへ渡ります</b>。'
  };
  ...
  var CHECKS = [
    ...
    /* ⭐F2 が切れた＝この旅で初めて「ヒューズが本当の犯人」になる本線 */
  ...
  Journey.boot({
    ...
    /* ⛔ストップスイッチと後コンビランプには印を打たない＝wiring-layout.json はオーナーの実測値
         だけでできていて、こちらの推定値を1つも入れていない（この2部品はまだ実測が無い）。
       ⚠️代わりに車の全長を出す＝この旅は「ダッシュのキー → 前のヒューズ箱 → 車の最後尾」まで
         行く長い旅なので、第3号（セル）と同じ広さの viewBox が要る。 */
    carmap: function () {
      return {
